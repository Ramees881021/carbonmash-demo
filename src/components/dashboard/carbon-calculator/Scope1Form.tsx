...
export const Scope1Form = ({ entries, onChange, onClearAll }: Scope1FormProps) => {
  ...
  const addEntry = (subCategory: Scope1Entry['subCategory'], type: string, quantity: number, description: string) => {
    ...
    if (subCategory === 'stationary') {
      ...
      if (fuel) { tco2e = quantity * fuel.factor; unit = fuel.unit; emissionFactor = fuel.factor; emissionFactorSource = `UK DEFRA 2026 – Stationary Combustion (${fuel.label})`; }
    } else if (subCategory === 'mobile') {
      const vehicle = VEHICLE_TYPES[type as keyof typeof VEHICLE_TYPES];
      if (vehicle) { tco2e = quantity * vehicle.factor; unit = vehicle.unit; emissionFactor = vehicle.factor; emissionFactorSource = `UK DEFRA 2026 – Mobile Combustion (${vehicle.label})`; }
