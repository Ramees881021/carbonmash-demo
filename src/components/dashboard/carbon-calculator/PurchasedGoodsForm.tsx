...
import { Plus, ShoppingCart, Globe, MapPin, CheckCircle, Circle, Upload, Download, FileSpreadsheet, X, Check, AlertCircle, Loader2 } from 'lucide-react';
...
export const PurchasedGoodsForm = ({ onAdd, onAddBatch, sites = [], entries = [] }: PurchasedGoodsFormProps) => {
  ...
  // Calculate emissions based on active method
  const calculatedEmission = useMemo(() => {
    switch (activeMethod) {
      case 'supplier':
        return apportionedEmission ? parseFloat(apportionedEmission) / 1000 : 0; // kg to tCO2e
      case 'average':
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, 'scope3_cat1_purchased_goods.xlsx');
  ...
  const handleAdd = async () => {
    ...
    onAdd({
      categoryCode: 'purchased_goods',
      type,
      quantity: qty,
      unit,
      tco2e,
      description: description || supplier || `Cat 1 - ${METHODS.find(m => m.key === activeMethod)?.label}`,
    ...
    <Card>
      <CardHeader>
        <CardTitle className="text-sm flex items-center gap-2">
          <ShoppingCart className="h-4 w-4" /> 1. Purchased Goods & Services
        </CardTitle>
      ...
      <CardContent className="space-y-5">
                ...
                {m.key === 'supplier' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div className="space-y-1.5">
                      ...
                      <Input
                        type="number"
                        value={apportionedEmission}
                        onChange={e => setApportionedEmission(e.target.value)}
                        placeholder="0"
                        className="h-9"
                      />
                    </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end">
          <div className="space-y-2">
            ...
            <Input value={description} onChange={e => setDescription(e.target.value)} placeholder="e.g. Office supplies Q1" />
          </div>
...
const BulkImportSection = ({ onAdd, onAddBatch, sites, mode, selectedSiteId }: {
  ...
  const downloadTemplate = () => {
    ...
    const sampleData = [
      TEMPLATE_COLUMNS,
      ['Acme Corp', 'supplier', '5000', '', '', 'Steel components'],
      ['Widget Inc', 'average', '', '100', '', 'Plastic parts'],
      ['Office Depot', 'spend', '', '', '25000', 'Office supplies'],
    ];
    const ws = XLSX.utils.aoa_to_sheet(sampleData);
    ws['!cols'] = TEMPLATE_COLUMNS.map(() => ({ wch: 22 }));
    XLSX.utils.book_append_sheet(wb, ws, 'Purchased Goods');
    XLSX.writeFile(wb, 'scope3_cat1_template.xlsx');
        ...
        const parsed: ParsedRow[] = dataRows.map((row) => {
          ...
          switch (method) {
            case 'supplier':
              ...
              break;
            case 'average':
              if (qty > 0) { tco2e = 0; } // AI will assign EF
              else { valid = false; error = 'Missing quantity'; }
              break;
            case 'spend':
              if (spend > 0) { tco2e = 0; } // AI will assign EF
  ...
  const handleImportAll = async () => {
    ...
    // Separate supplier rows from AI rows
    const supplierRows = validRows.filter(r => r.method === 'supplier');
    const aiRows = validRows.filter(r => r.method !== 'supplier');

    // Add supplier rows directly
    supplierRows.forEach(row => {
      allNewEntries.push({ categoryCode: 'purchased_goods', type: 'supplier_specific', quantity: row.apportionedEmission || 0, unit: 'kg CO₂e', tco2e: row.tco2e, description: row.description, siteId: mode === 'site' ? selectedSiteId : null });
    });

    // Call AI for average/spend rows in batch
    if (aiRows.length > 0) {
      try {
        const { data, error } = await supabase.functions.invoke('assign-emission-factor', {
          body: {
        ...
        aiRows.forEach((row, i) => {
          ...
          allNewEntries.push({ categoryCode: 'purchased_goods', type, quantity: qty, unit, tco2e, description: row.description, siteId: mode === 'site' ? selectedSiteId : null });
        });
        ...
        aiRows.forEach(row => {
          ...
          const qty = row.method === 'average' ? (row.quantity || 0) : (row.totalSpend || 0);
          const unit = row.method === 'average' ? 'units' : '$';
          allNewEntries.push({ categoryCode: 'purchased_goods', type, quantity: qty, unit, tco2e: 0, description: row.description, siteId: mode === 'site' ? selectedSiteId : null });
        });
        toast.warning(`Imported ${validRows.length} entries but AI EF assignment failed. Some entries have 0 tCO₂e.`);
      }
    } else {
    ...
    console.log('[BulkImport PG] allNewEntries count:', allNewEntries.length, allNewEntries);
    if (onAddBatch) {
      console.log('[BulkImport PG] Using onAddBatch');
      onAddBatch(allNewEntries);
    } else {
      console.log('[BulkImport PG] Falling back to onAdd loop');
