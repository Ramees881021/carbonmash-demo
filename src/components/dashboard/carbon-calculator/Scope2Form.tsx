...
export const Scope2Form = ({ entries, onChange, site, onClearAll }: Scope2FormProps) => {
  ...
  const addElectricity = (kwh: number, renewablePct: number, method: 'location' | 'market', desc: string, customRegion?: string) => {
    ...
    onChange([...entries, {
      ...
      emissionFactor: factor, emissionFactorSource: `IEA/DEFRA 2026 – Grid Electricity (${regionLabel})`,
    }]);
  };
  ...
  const addHeatSteam = (kwh: number, factor: number, desc: string) => {
    onChange([...entries, {
      ...
      emissionFactor: factor, emissionFactorSource: 'UK DEFRA 2026 – Heat & Steam',
    }]);
  };
