...
export const seedUserMasterData = async (userId: string, email?: string | null, companyName?: string | null) => {
  ...
  const isAccountMismatch = existingProfiles.length > 0 && 
    (existingProfiles[0]?.company_name !== targetCompanyName);
  ...
  for (const [colName, records] of Object.entries(masterData)) {
    ...
    const needsReseed = existing.length === 0 || isAccountMismatch;
...
class FirestoreQueryBuilder<T = any> implements PromiseLike<{ data: T[] | null; error: any; count?: number }> {
  ...
  private async execute(): Promise<{ data: T[]; error: null }> {
    ...
    const hasAccountMismatch = cachedProfiles.length > 0 && expectedProfile && 
      cachedProfiles[0].company_name !== expectedCompanyName;
    ...
    const needsReseed = existingUserRecords.length === 0 || hasAccountMismatch || isEmissionsMismatch;
